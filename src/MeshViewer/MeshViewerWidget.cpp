...
bool MeshViewerWidget::LoadMesh(const std::string & filename)
{
	...
	if (read_OK)
	{
		acamcad::polymesh::loadMesh(filename, storedMesh);
		...
		// ³õÊ¼»¯Ëã·¨
		eigenAnalysis.reset( new EigensystemDistortion(polyMesh, storedMesh));
		return true;
	}
	return false;
}
...
void MeshViewerWidget::ChangeBoundaryType(int boundaryType) {
	this->boundaryType =(BoundaryType) boundaryType;
	std::cout <<"Boundary Type Changed: " << boundaryType << std::endl;
}

void MeshViewerWidget::DoTuttleFlatten(void) {
	FixedBoundaryParaAlg::TuttleParametrization(polyMesh, storedMesh, boundaryType);
	isParamerized = true;
	update();
}

// preserve boundary
void MeshViewerWidget::DoFloaterFlatten(void) {
	FixedBoundaryParaAlg::FloaterParametrization(polyMesh, storedMesh, boundaryType);
	isParamerized = true;
	update();
}

void MeshViewerWidget::DoARAPOpt(void) {
	if (!isParamerized) {
		FixedBoundaryParaAlg::FloaterParametrization(polyMesh, storedMesh, BoundaryType::Circle);
		isParamerized = true;
	}
	this->eigenAnalysis->Init();
	this->eigenAnalysis->energyType = EigensystemDistortion::EnergyType::ARAP;
	this->eigenAnalysis->DoForward();

	update();
}

void MeshViewerWidget::DoSDOpt(void) {
	if (!isParamerized) {
		FixedBoundaryParaAlg::FloaterParametrization(polyMesh, storedMesh, BoundaryType::Circle);
		isParamerized = true;
	}
	this->eigenAnalysis->Init();
	this->eigenAnalysis->energyType = EigensystemDistortion::EnergyType::SD;
	this->eigenAnalysis->DoForward();
	update();
}

void MeshViewerWidget::DoLSCM(void) {
	LSCM::LSCMParametrization(this->polyMesh);
	update();
}

void MeshViewerWidget::DoDOF(void) {
	LSCM::LSCMParametrization(this->polyMesh);
	FixedBoundaryParaAlg::TuttleParametrization(polyMesh,storedMesh, BoundaryType::Current);
	update();
}
